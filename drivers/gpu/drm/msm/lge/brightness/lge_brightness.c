...
int lge_backlight_device_update_status(struct backlight_device *bd)
{
	...
	} else if (panel->lge.br_enhance){
		bl_type = LGE_BLMAP_BR_ENHANCE;
	} else {
		bl_type = LGE_BLMAP_DEFAULT;
	}
#if defined(CONFIG_LGE_DISPLAY_VR_LP_BLMAP)
	if(panel->lge.vr_low_persist_enable)
