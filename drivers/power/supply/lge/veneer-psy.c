...
#if defined(CONFIG_MACH_SDM845_CAYMANSLM)
static struct power_supply* get_psy_dc(struct veneer* veneer_me) {
	if (!veneer_me->psy_dc)
...
static enum charging_supplier supplier_factory(/* @Nonnull */ struct power_supply* usb,
	/* @Nonnull */ union power_supply_propval* buf) {
	enum charging_supplier ret = CHARGING_SUPPLY_TYPE_UNKNOWN;
	if (buf->intval == POWER_SUPPLY_TYPE_USB)
		ret = CHARGING_SUPPLY_USB_2P0;
...
#if defined(CONFIG_LGE_TOUCH_CORE) || defined(CONFIG_LGE_TOUCH_CORE_QOS)
...
#if defined(CONFIG_MACH_SDM845_CAYMANSLM)
	struct power_supply* psy_dc = NULL;
#endif
...
#if defined(CONFIG_MACH_SDM845_CAYMANSLM)
			rc = power_supply_get_property(psy_dc,
...
int set_veneer_param(int id, int val)
{
	...
	switch (id) {
		case VENEER_FEED_VDC: /* mV */
#if defined(CONFIG_MACH_SDM845_CAYMANSLM)
			prop.intval = val;  /* no unit convert : mV */
			rc = power_supply_set_property(psy_wireless,
				POWER_SUPPLY_PROP_VOLTAGE_MAX, &prop);
#else
			prop.intval = val * 1000;  /* unit convert from mV to uV */
			rc = power_supply_set_property(psy_wireless,
				POWER_SUPPLY_PROP_VOLTAGE_MAX_DESIGN, &prop);
#endif
			break;
